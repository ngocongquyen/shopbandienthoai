function cartQntAndPrice() {
    let inpEl = document.querySelector('.product__qntInput'); // get element of input
    let qntValue = Number(inpEl.getAttribute('value')); // get value of input
    let decBtn = document.querySelector('.product__qnt-btn--dec'); // get element of decrease button
    let incBtn = document.querySelector('.product__qnt-btn--inc'); // get element of increase button
    ...
    // let maxQnt = 1100;
    var product_limit = document.querySelector('.product_limit_order');
    incBtn.onclick = (e) => {
        ...
        if (qntValue < maxQnt) {
            ...
            $('#postnumber').val(qntValue);
        }
        else {
            product_limit.style.display = 'block';
        }
        e.preventDefault();
    }   
    ...
    decBtn.onclick = (e) => {
        if (qntValue > 1) {
            ...
            inpEl.value = qntValue;
            $('#postnumber').val(qntValue);
            product_limit.style.display = 'none';
        }
        e.preventDefault(); 
    }
