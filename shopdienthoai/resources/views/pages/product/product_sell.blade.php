         ...
         <div class="home-content">
            <div class="row">
               <div class="home-filter hide-on-mobile-table col">
                  ...
                  <button class="home-filter__btn btn btn--primary " onclick="document.location.href=this.getAttribute('href');" href="{{URL::to('product-selling')}}">Bán chạy</button>
               ...
               @foreach($product_selling as $key => $product_sell)
                  <input type="hidden" id="wishlist_productname{{$product_sell->product_id}}"  value="{{$product_sell->product_name}}">
                  <input type="hidden" id="wishlist_producturl{{$product_sell->product_id}}"   value="{{url('chi-tiet-san-pham/'.$product_sell->product_id)}}">
                  <input type="hidden" id="wishlist_productprice{{$product_sell->product_id}}" value="{{number_format($product_sell->product_price-($product_sell->product_price*($product_sell->promotion_type/100)),0,',','.')}}">
                  <input type="hidden" id="wishlist_productimage{{$product_sell->product_id}}" value="{{asset('public/uploads/product/'.$product_sell->product_images)}}">
                  <input type="hidden" id="wishlist_producttype{{$product_sell->product_id}}" value="{{$product_sell->promotion_type}}">
                  <input type="hidden" id="wishlist_productsold{{$product_sell->product_id}}" value="{{$product_sell->product_sold}}">
                  <div class="col l-3 m-6 c-6">
                     <div class="home-product-item" >
                        <a href="{{URL::to('/chi-tiet-san-pham/'.$product_sell->product_id)}}">
                           <img src="{{URL::to('public/uploads/product/'.$product_sell->product_images)}}" class="home-product-item__img">
                        </a>
                        <h4 class="home-product-item__name">{{$product_sell->product_name}}</h4>
                        <div class="home-product-item__price">
                           <span class="home-product-item__price-old">{{number_format($product_sell->product_price,0,',','.')}} đ</span>
                           <span class="home-product-item__price-new">{{number_format($product_sell->product_price-($product_sell->product_price*($product_sell->promotion_type/100)),0,",",".") }}đ</span>
                        </div>
                        <div class="home-product-item__action">
                           <span class="home-product-item__like home-product-item__like--liked">
                           <i class="home-product-item__like-icon-empty far fa-heart" id="{{$product_sell->product_id}}" onclick="add_wistlist(this.id)"></i>
                           <i class="home-product-item__like-icon-fill fas fa-heart home-product-item__liked"></i>
                           </span>
                           <?php
                                $number_star = 0;
                                if($product_sell->product_total_rating) {
                                    $number_star = round($product_sell->product_total_number / $product_sell->product_total_rating,2); 
                                }
                            ?>
                           ...
                           <span class="home-product-item__sold">{{$product_sell->product_sold}} đã bán</span>
                        </div>
                        ...
                        @if($product_sell->promotion_type !=0 ) 
                            <div class="home-product-item__sell-off">
                            <span class="home-product-item__sell-off-percent">{{$product_sell->promotion_type}}%</span>
